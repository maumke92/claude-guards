...
def lauf(nutzlast):
    ...
    p = subprocess.run(
        [sys.executable, str(HIER / "repeat-guard.py")],
        input=nutzlast,
        capture_output=True,
        text=True,
    )
...
# --- Signaturen ---------------------------------------------------------
check(
    "gleicher Bash-Befehl trotz anderer Leerzeichen gleiche Signatur",
    rg.signature("Bash", {"command": "ls  -la"})
    == rg.signature("Bash", {"command": "ls -la"}),
)
check(
    "anderer Bash-Befehl andere Signatur",
    rg.signature("Bash", {"command": "ls -la"})
    != rg.signature("Bash", {"command": "ls -l"}),
)
check(
    "Read auf gleiche Datei ist gleich, egal ab welcher Zeile",
    rg.signature("Read", {"file_path": "a.py", "offset": 1})
    == rg.signature("Read", {"file_path": "a.py", "offset": 500}),
)
check(
    "Edit mit anderem old_string andere Signatur",
    rg.signature("Edit", {"file_path": "a.py", "old_string": "x"})
    != rg.signature("Edit", {"file_path": "a.py", "old_string": "y"}),
)
check(
    "leerer Befehl liefert keine Signatur",
    rg.signature("Bash", {"command": "   "}) is None,
)
check("fehlendes tool_input liefert keine Signatur", rg.signature("Read", None) is None)
check(
    "unbekanntes Werkzeug bekommt trotzdem eine Signatur",
...
z2 = {}
check(
    "drei verschiedene Aufrufe erinnern nie",
    not any(
...
# --- Statusdatei --------------------------------------------------------
check(
    "Sitzungskennung wird fuer den Dateinamen entschaerft",
    ".." not in rg.statusdatei("../../boese").name
    and "/" not in rg.statusdatei("a/b").name,
)
check(
    "fehlende Sitzungskennung faellt auf einen festen Namen zurueck",
    rg.statusdatei(None).name.endswith(".json"),
)
check(
    "fehlende Statusdatei wird verworfen statt zu werfen",
...
check("Zaehlen laeuft danach ohne Ausnahme durch", rg.zaehle(gelesen, "aaa") is False)
...
code, aus = lauf(
    json.dumps(
        {"session_id": "t", "tool_name": "Bash", "tool_input": {"command": "   "}}
    )
)
check("Aufruf ohne Inhalt: Exit 0 ohne Ausgabe", code == 0 and aus == "")

sitzung = f"selbsttest-{id(object())}"
rg.statusdatei(sitzung).unlink(missing_ok=True)
nutzlast = json.dumps(
    {"session_id": sitzung, "tool_name": "Read", "tool_input": {"file_path": "app.py"}}
)
ausgaben = [lauf(nutzlast) for _ in range(4)]
check(
    "echter Hook meldet sich genau einmal",
...
check(
    "Ausgabe traegt Kontext fuer das Modell",
...
check(
    "kein Temp-Rest nach atomarem Schreiben",
    not list(rg.statusdatei(sitzung).parent.glob(f"repeat-{sitzung}*.tmp")),
)
rg.statusdatei(sitzung).unlink(missing_ok=True)
